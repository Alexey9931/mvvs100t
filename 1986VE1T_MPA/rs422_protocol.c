...
uint8_t receive_packet(UARTn *UART_struct, uint8_t ext_bus)
{
	...
	data_exchange_errors protocol_error;
	...
	if (uart_error != 0)
	{
		uart_error_handler(uart_error, ext_bus);
		protocol_error = UART_ERROR;
...
/*
Функция обработчика ошибок работы UART
*/
void uart_error_handler(uart_errors uart_error, uint8_t ext_bus)
{
	switch ((uint8_t)uart_error)
	{
		case INIT_ERROR:
			break;
...
/*
Функция обработчика ошибок протокола
*/
